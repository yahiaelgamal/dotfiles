...
angled_x_axis <- function(angle = 60) {
  ggplot2::theme(
    axis.text.x =
      ggplot2::element_text(angle = angle, hjust = 1)
  )
}

# This method calculates the avg cardinality (cardinality being how many values
# does one value in column c1 coexist in column c2). If the avg cardinality is
...
# (maybe) should be considered as hierarchical. As there might be naming
# conflicts
avg_cardinality_matrix <- function(df) {
  ...
  for (c1 in colnames(df)) {
    ...
    for (c2 in colnames(df)) {
      cc <- df %>%
        group_by(!!sym(c1)) %>%
        summarise(!!sym(paste0("len_", c2)) := length(unique(!!sym(c2))))
